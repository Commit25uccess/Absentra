...
import { createLeaveRequest, cancelLeaveRequest } from '../services/leave-request.service';
import { hasSufficientBalance } from '../services/balance.service';
import { getLeaveTypeById } from '../services/leave-type.service';
import { parseSlackDate } from '../utils/dates';
import {
...
export function registerLeaveRequestActions(app: App): void {
  ...
  app.action('leave_duration', async ({ ack, body, client }) => {
    ...
    try {
      ...
      logger.error({ event: 'modal_update_failed', userId, action: 'duration_change' }, error);
    }
  });
  ...
  app.action('leave_type', async ({ ack, body, client }) => {
    ...
    try {
      ...
      logger.error({ event: 'modal_update_failed', userId, action: 'type_change' }, error);
    }
  });
  ...
  app.action('start_date', async ({ ack, body, client }) => {
    ...
    if (state.duration !== 'multiple_days') {
      state.endDate = state.startDate ?? undefined;
    }

    try {
      await updateModalWithWarnings(client, view.id, userId, state);
    } catch (error) {
      logger.error({ event: 'modal_update_failed', userId, action: 'start_date_change' }, error);
    }
  });
  ...
  app.action('end_date', async ({ ack, body, client }) => {
    ...
    try {
      ...
      logger.error({ event: 'modal_update_failed', userId, action: 'end_date_change' }, error);
    }
  });
  ...
  app.view('leave_request_submit', async ({ ack, body, view, client }) => {
    ...
    try {
      ...
      const request = await createLeaveRequest({
        ...
        endDate,
        isHalfDay,
        halfDayPeriod: isHalfDay && halfDayPeriod ? (halfDayPeriod as 'morning' | 'afternoon') : undefined,
        reason: reason!,
      ...
      await ack({
        ...
        view: await buildLeaveRequestConfirmation(
          request.leaveType.name,
          request.leaveType.emoji,
          request.startDate,
          request.endDate,
          request.totalDays.toNumber(),
          request.status,
          requiresApproval
        ),
      });
      ...
      refreshUserHomeTab(client, userId).catch(_err =>
        logger.warn({ event: 'home_tab_refresh_failed' })
      );
    } catch (error) {
      logger.error({ event: 'leave_request_submission_failed', userId }, error);
      await ack({
        response_action: 'errors',
        errors: {
  ...
  app.action('cancel_request', async ({ ack, body, client, action }) => {
    ...
    try {
      ...
      logger.error({ event: 'request_cancellation_failed', userId, requestId }, error);
      await client.chat.postEphemeral({
        channel: (body as any).channel?.id || userId,
        user: userId,
