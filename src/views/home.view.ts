...
import { getCurrentYear, formatDate, getToday } from '../utils/dates';
...
export function buildHomeView(data: HomeViewData): HomeView {
  ...
  if (isManager || isAdmin) {
    ...
    blocks.push(
      ...
      section(pendingText),
      actions([
        button('⏳ Review Pending', 'home_pending_requests', {
  ...
  // Who's Out Today
  blocks.push(
    section(`*Who's Out Today* - ${formatDate(today, 'EEEE, MMM d')}`),
  );
  ...
  if (futureLeaves.length > 0) {
    ...
    const upcomingList = futureLeaves.slice(0, PAGINATION.MAX_PREVIEW_ITEMS).map((r) =>
      `${r.leaveType.emoji} <@${r.requester.slackId}> - ${formatDate(r.startDate, 'EEE, MMM d')}`
    ).join('\n');

    blocks.push(
      section(upcomingList),
      divider(),
    );
  }
  ...
  blocks.push(
    context([
      `_Last updated: ${formatDate(new Date(), 'MMM d, yyyy h:mm a')}_ • Type \`/pto help\` for commands`,
    ]),
  );
