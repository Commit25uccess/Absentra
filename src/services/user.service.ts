...
export async function getOrCreateUser(slackUser: SlackUser): Promise<User> {
  ...
  try {
    ...
    userLogger.debug('User synced', { slackId: slackUser.id, displayName });

    return user;
  } catch (error) {
    userLogger.error('Failed to get or create user', error, { slackId: slackUser.id });
    throw error;
  }
}
...
export async function setUserAdmin(slackId: string, isAdmin: boolean): Promise<User> {
  ...
  userLogger.info('User admin status updated', { slackId, isAdmin });
...
export async function assignUserToTeam(
  ...
  userLogger.info('User assigned to team', { slackId: userSlackId, teamId });
...
async function ensureUserBalances(userId: string): Promise<void> {
  ...
  if (newBalances.length > 0) {
    ...
    userLogger.debug('Created leave balances for user', {
      userId,
...
export async function syncWorkspaceUsers(client: any): Promise<User[]> {
  userLogger.info('Starting workspace user sync');
  ...
  try {
    ...
    if (!result.members) {
      userLogger.warn('No members returned from Slack API');
      return [];
    }
    ...
    userLogger.debug('Filtered real users from Slack', {
    ...
    userLogger.info('Workspace user sync completed', {
    ...
    userLogger.error('Failed to sync workspace users', error);
