...
export async function refreshUserHomeTab(
  ...
  slackLogger.debug('Home tab refreshed', { userSlackId, duration });
}
...
export async function safeViewOpen(
  ...
  try {
    ...
    slackLogger.debug('View opened successfully', { method, callbackId: view.callback_id });
    return true;
  } catch (error) {
    // Handle expired trigger_id gracefully
    if (isExpiredTriggerError(error)) {
      slackLogger.warn('View open failed: expired trigger ID', { method });
      ...
      if (fallback) {
        try {
          ...
          slackLogger.debug('Failed to send expired trigger fallback message', {
    ...
    slackLogger.error('View open failed', error, { method });
    throw error;
  }
}
...
export async function handleExpiredTrigger(
  ...
  slackLogger.warn('Handling expired trigger error', { userId });
  ...
  try {
    ...
    slackLogger.debug('Failed to send expired trigger message', {
      userId,
      error: ephemeralError instanceof Error ? ephemeralError.message : 'Unknown',
    });
  }
...
export async function sendEphemeral(
  ...
  try {
    ...
    slackLogger.error('Failed to send ephemeral message', error, {
      userId: options.user,
