import type { App, RespondFn, SlackCommandMiddlewareArgs, AllMiddlewareArgs } from '@slack/bolt';
...
export function registerLeaveCommand(app: App): void {
  app.command('/pto', async ({ command, ack, client, respond }) => {
    ...
    cmdLogger.debug('Processing /pto command', {
      userId,
      subcommand,
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      cmdLogger.error('Command failed', error, {
        command: '/pto',
        subcommand,
        userId,
        duration,
  ...
  cmdLogger.info('Registered /pto command');
}
...
async function handleRequest(
  ...
  try {
    ...
    cmdLogger.debug('Opened leave request modal', { userId });
  } catch (error) {
    if (isExpiredTriggerError(error)) {
      cmdLogger.warn('Leave request modal failed: expired trigger', { userId });
      await respond({
        response_type: 'ephemeral',
...
async function handleBalance(respond: RespondFn, userId: string): Promise<void> {
  ...
  cmdLogger.debug('Fetched user balances', {
    userId,
...
async function handleMyRequests(respond: RespondFn, userId: string): Promise<void> {
  ...
  cmdLogger.debug('Fetched user requests', {
    userId,
...
async function handleHistory(respond: RespondFn, userId: string): Promise<void> {
  ...
  cmdLogger.debug('Fetched user history', {
    userId,
    requestCount: requests.length,
  });
  ...
  await respond({
    response_type: 'ephemeral',
    blocks: buildMyRequestsMessage(requests, true),
  });
}
...
async function handleWhosOut(respond: RespondFn, period?: string): Promise<void> {
  if (period === 'week' || period === 'upcoming') {
    ...
    cmdLogger.debug('Fetched upcoming leaves', {
      period: 'week',
    ...
    await respond({
      response_type: 'ephemeral',
      blocks: buildUpcomingLeavesMessage(requests, 7),
    });
  } else {
    const requests = await getWhosOut();

    cmdLogger.debug('Fetched who is out today', {
      period: 'today',
      count: requests.length,
    });
...
async function handlePending(respond: RespondFn, userId: string): Promise<void> {
  ...
  if (!isManager && !isAdmin) {
    cmdLogger.debug('Pending requests denied: not manager/admin', { userId });
  ...
  cmdLogger.debug('Fetched pending requests', {
    userId,
    isManager,
    isAdmin,
...
async function handleAdmin(respond: RespondFn, userId: string, args: string[]): Promise<void> {
  ...
  if (!isAdmin) {
    cmdLogger.debug('Admin command denied: not admin', { userId });
  ...
  cmdLogger.debug('Showing admin help', { userId, args });
...
async function handleHelp(respond: RespondFn, userId: string): Promise<void> {
  ...
  cmdLogger.debug('Showing help', { userId, isManager, isAdmin });
